...
def check_wallet_mm(driver, mm_address):
    ...
    try:
        ...
        logger.debug(
            f" (check_wallet_mm) Проверяем страницу на btn [Copy to clipboard]"
        )
        driver.implicitly_wait(10)
        WebDriverWait(driver, 15).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
        ).click()
...
def check_mm_data_base(driver, mm_address, row, workbook_mm, worksheet_mm, FILE_PATH):
    ...
    if wallet_mm_from_browser_extension == mm_address:
        logger.warning(
            f" (check_wallet_mm) Адреса в БД и в расширениях браузера МетаМаск совпадают. OK!"
        )
        return wallet_mm_from_browser_extension
    elif (
        mm_address
        and wallet_mm_from_browser_extension
        and mm_address != wallet_mm_from_browser_extension
    ):
        logger.warning(
        ...
        worksheet_mm.cell(row=row + 1, column=4).value = (
            wallet_mm_from_browser_extension
        )
        workbook_mm.save(FILE_PATH)
        logger.warning(
            f" (check_mm_data_base) Profile №: {row}, Адрес МетаМаск в БД исправлен!"
        )
        return wallet_mm_from_browser_extension
    else:
        if not mm_address and wallet_mm_from_browser_extension:
            logger.warning(
                f" (check_mm_data_base) Profile №: {row}, Адрес МетаМаск в БД отсутствует"
            )
            worksheet_mm.cell(row=row + 1, column=4).value = (
                wallet_mm_from_browser_extension
            )
            workbook_mm.save(FILE_PATH)
            logger.warning(
                f" (check_mm_data_base) Profile №: {row}, Адрес МетаМаск в БД добавлен!"
            )
            return wallet_mm_from_browser_extension
    ...
    def check_version_mm(driver):
        try:
            ...
            WebDriverWait(driver, 10).until(
                EC.visibility_of_element_located((By.CLASS_NAME, path_class_name))
            )
...
def click_unlock_button(driver):
    try:
        unlock_button = driver.find_element(
            By.CSS_SELECTOR, '[data-testid="unlock-submit"]'
        )
        # Нажатие на кнопку
        unlock_button.click()
...
def check_page_url(
    driver, url="chrome-extension://nkbihfbeogaeaoehlefnkodbefgpgknn/home.html#"
):
...
def check_password_error(driver):
    if check_page_url(driver):
        ...
        try:
            ...
            logger.debug(
                " (check_password_error) Проверяем наличие правильного пароля..."
            )
...
def click_forgot_password(driver):
    try:
        ...
        logger.debug(
            " (click_forgot_password) Кнопка [Forgot password?] успешно нажата"
        )
        return True
    except Exception:
...
def handle_incorrect_password(driver):
    if not check_password_error(driver):
        logger.debug(
            " (handle_incorrect_password) Ошибка пароля обнаружена, нажимаем [Forgot password?]..."
        )
        if click_forgot_password(driver):
            logger.debug(
                " (handle_incorrect_password) Нажата кнопка [Forgot password?] успешно."
            )
            return True
        else:
            logger.error(
                " (handle_incorrect_password) Не удалось нажать на [Forgot password?]."
            )
    else:
...
def fill_seed(driver, seed):
    try:
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, "import-srp__srp-word-0"))
        )
    ...
    if platform.system() == "Darwin":
        ...
        pyperclip.copy(seed)
        ActionChains(driver).key_down("\ue03d").send_keys("v").key_up(
            "\ue03d"
        ).perform()
    ...
    def input_password(driver, password):
        ...
        def confirm_password(driver, password):
            try:
                ...
                seed_phrase_input.send_keys(password)
                logger.debug(
                    f" (confirm_password) Confirm password успешно введен: {password}"
                )
                return True
            except Exception:
...
def get_started(driver, env_id):
    if check_page_url(
        ...
        try:
            ...
            if WebDriverWait(driver, 35).until(
                EC.visibility_of_element_located((By.CLASS_NAME, path_class))
            ):
                logger.info(
                    " (get_started) onboarding-welcome page. Let's get started..."
                )
                return True
        except NoSuchElementException as e:
            traceback.print_exc()
...
def onboard_page(driver, seed, password):  # Import exist wallet
    def agree_checkbox(driver):
        try:  # checkbox: I agree to MetaMask's Terms of use
            ...
            logger.debug(
                f" (agree_checkbox) Проверяем страницу на присутствие checkbox: {path_id}"
            )
            WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.ID, path_id))
            ).click()
    ...
    def click_on_import_exist_wallet(driver):
        try:  # Press btn [Import exist wallet]
            ...
            path_css_selector = f"[data-testid={data_testid}]"
            logger.debug(
                f" (click_on_import_exist_wallet) Проверяем страницу на присутствие btn [Import exist wallet]"
            )
            WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
            ).click()
            logger.debug(
                f" (click_on_import_exist_wallet) Нажали кнопку btn [Import exist wallet]"
            )
        except Exception:
    ...
    def metametrics(driver):
        try:  # Button [No thanks]
            ...
            path_css_selector = f"[data-testid={data_testid}]"
            logger.debug(
                f" (metametrics) Проверяем страницу на присутствие btn [No thanks]"
            )
            WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
            ).click()
    ...
    def btn_confirm_secret_recovery_phrase(
        driver,
    ):  # Button [Confirm Secret Recovery Phrase]
        try:
            data_testid = "import-srp-confirm"
            path_css_selector = f"[data-testid={data_testid}]"
            logger.debug(
                f" (btn_confirm_secret_recovery_phrase) Проверяем страницу на присутствие btn [Confirm Secret Recovery Phrase]"
            )
            WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
            ).click()
            logger.debug(
                f" (btn_confirm_secret_recovery_phrase) Нажали кнопку btn [Confirm Secret Recovery Phrase]"
            )
        except Exception:
                ...
                if (
                    ...
                    .until(
                        EC.visibility_of_element_located(
                            (By.CSS_SELECTOR, path_css_selector)
                        )
                    )
                    .text
                ):
                    logger.debug(
                        f" (onboarding_create_password_check_page) On page onboarding/create-password exists"
                    )
                    return True
            except Exception:
        ...
        if onboarding_create_password_check_page(driver):
            ...
            def input_password(driver, password):
                def new_password(driver, password):
                    try:
                        ...
                        path_css_selector = f"[data-testid={data_testid}]"
                        logger.debug(
                            f" (new_password) Проверяем страницу на присутствие поля ввода New password"
                        )
                        ...
                        logger.debug(
                            f" (new_password) New password успешно введен: {password} onboarding/create-password"
                        )
                        return True
                    except Exception:
                        logger.error(f" (new_password) Exception")
                ...
                def confirm_password(driver, password):
                    try:
                        ...
                        path_css_selector = f"[data-testid={data_testid}]"
                        logger.debug(
                            f" (confirm_password) Проверяем страницу на присутствие поля ввода Confirm password"
                        )
                        ...
                        logger.debug(
                            f" (confirm_password) Confirm password успешно введен: {password} onboarding/create-password"
                        )
                        return True
                    except Exception:
                        logger.error(f" (confirm_password) Exception")
            ...
            def checkbox_mm_cannot_recover(driver):
                try:
                    ...
                    path_css_selector = f"[data-testid={data_testid}]"
                    logger.debug(
                        f" (checkbox_mm_cannot_recover) Проверяем страницу на присутствие btn [Import exist wallet]"
                    )
                    WebDriverWait(driver, 10).until(
                        EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
                    ).click()
            ...
            def click_import_my_wallet_btn(driver):
                try:
                    ...
                    path_css_selector = f"[data-testid={data_testid}]"
                    logger.debug(
                        f" (click_import_my_wallet_btn) Проверяем страницу на присутствие btn [Import my wallet]"
                    )
                    WebDriverWait(driver, 10).until(
                        EC.element_to_be_clickable((By.CSS_SELECTOR, path_css_selector))
                    ).click()
                    logger.debug(
                        f" (click_import_my_wallet_btn) Нажали кнопку [Import my wallet]"
                    )
                except Exception:
            ...
            def your_wallet_is_ready_check_page(driver):  # Your wallet is ready
                try:
                    ...
                    # Ожидание видимости элемента
                    logger.debug(
                        f" (your_wallet_is_ready_check_page) Checking on page onboarding/completion"
                    )
                    if (
                        WebDriverWait(driver, 10)
                        .until(
                        ...
                        logger.debug(
                            f" (your_wallet_is_ready_check_page) Page onboarding/completion exists"
                        )
                        return True
                except Exception:
            ...
            if your_wallet_is_ready_check_page(driver):
                ...
                def click_on_done_btn(driver):
                    try:
                        ...
                        path_css_selector = f"[data-testid={data_testid}]"
                        logger.debug(
                            f" (click_on_done_btn) Проверяем страницу на присутствие btn [Done]"
                        )
                        WebDriverWait(driver, 10).until(
            ...
            def pin_extension_check_page(driver):
                try:
                    ...
                    # Ожидание видимости элемента
                    logger.debug(
                        f" (pin_extension_check_page) Checking on page onboarding/pin-extension"
                    )
                    if (
                        WebDriverWait(driver, 10)
                        .until(
                        ...
                        logger.debug(
                            f" (pin_extension_check_page) Page onboarding/pin-extension exists"
                        )
                        return True
                except Exception:
            ...
            if pin_extension_check_page(driver):
                ...
                def click_on_pin_extension_btn_next(driver):  # pin-extension btn [Next]
                    try:
                        ...
                        path_css_selector = f"[data-testid={data_testid}]"
                        logger.debug(
                            f" (click_on_pin_extension_btn_next) Проверяем страницу на присутствие btn [Next]"
                        )
                        WebDriverWait(driver, 10).until(
                            EC.element_to_be_clickable(
                                (By.CSS_SELECTOR, path_css_selector)
                        ...
                        logger.debug(
                            f" (click_on_pin_extension_btn_next) Нажали кнопку btn [Next]"
                        )
                    except Exception:
                ...
                def click_on_pin_extension_btn_done(driver):  # pin-extension btn [Done]
                    try:
                        ...
                        path_css_selector = f"[data-testid={data_testid}]"
                        logger.debug(
                            f" (click_on_pin_extension_btn_done) Проверяем страницу на присутствие btn [Done]"
                        )
                        WebDriverWait(driver, 10).until(
                            EC.element_to_be_clickable(
                                (By.CSS_SELECTOR, path_css_selector)
                        ...
                        logger.debug(
                            f" (click_on_pin_extension_btn_done) Нажали кнопку btn [Done]"
                        )
                    except Exception:
            ...
            logger.info(
                " (onboarding_create_password_check_page) Successfully imported wallet!"
            )
...
def starting_metamask(driver, seed, password, env_id):
    delete_others_windows(driver)
    open_tab(
        driver, "chrome-extension://nkbihfbeogaeaoehlefnkodbefgpgknn/home.html#unlock"
    )
    ...
    if unlock(driver):
        ...
        if check_page_url(driver):
            ...
            if handle_incorrect_password(driver):
                logger.info(
                    " (starting_metamask), (unlock), handle_incorrect_password True\n"
                ...
                logger.info(
                    " (starting_metamask), (unlock), (handle_incorrect_password), "
