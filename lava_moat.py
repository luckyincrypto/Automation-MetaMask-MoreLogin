...
def modify_file_runtimelavamoat(env_id):
    ...
    while True:
        # logger.debug(" (modify_file_runtimelavamoat) Проверяем наличие последней версий MetaMask.")
    ...
    if ('const {' in lines[94] and
        ...
        # print("(modify_file_runtimelavamoat) Изменения в файле runtime-lavamoat.js успешно сохранены.")
        logger.info(" (modify_file_runtimelavamoat) Изменения в файле runtime-lavamoat.js успешно сохранены.")
        return True
    elif ('const {' in lines[94] and
            'scuttleGlobalThis,' in lines[95] and
            '} = {"scuttleGlobalThis":{"enabled":false,' in lines[96]):
        logger.info(f" (modify_file_runtimelavamoat) файл runtime-lavamoat.js в кэше профиля Env ID: {env_id} "
