...
#  Click button <text....>
def press_button(driver, text_btn):
    element = SeleniumUtilities.find_button_by_text(driver, text_btn)
    text_element = element.text
    if element and text_btn == text_element:
        logger.info(f' (press_button_connect_wallet), Found element with text "{text_element}"')
        if not SeleniumUtilities.click_safely(element):
            logger.error(f' (press_button_connect_wallet), Failed to click element with text {text_element}')
            return False
        logger.info(f' (press_button_connect_wallet), Clicked on the element with text {text_element}')
        return True
    logger.error(f' (press_button_connect_wallet), Element with text {text_btn} not found!')
    return False
...
class Onchaingm:
    ...
    DEPLOY_URL = 'https://onchaingm.com/deploy'
    HOME_URL = 'https://onchaingm.com/'


    def __init__(self, driver):
        """
        Инициализация Onchaingm.
        Args:
    ...
    def _open_website(self) -> bool | None:
        ...
        try:
            ...
            logger.debug(f'Opening {self.HOME_URL} website')
            ...
            if tab_tag_name == self.HOME_URL:
                logger.info(f' (open_website), Website opened on page: {tab_tag_name}')
                return True
            else:
                logger.info(f' (open_website), Website opened on page: {tab_tag_name}')
                return False
            ...
            logger.error(f' (open_website), Error opening {self.HOME_URL} website: {str(e)}')
            return None
    ...
    def onchaingm(self):
        """
        Обрабатывает активность Onchaingm.
        Returns:
        ...
        try:
            ...
            if not self._open_website():
                logger.debug(f" (onchaingm), Opened not on page: {self.HOME_URL}")
            else:
                logger.debug(f" (onchaingm), Opened on page: {self.HOME_URL}")

            current_windows = self.driver.window_handles
            logger.debug(f'Current windows: {current_windows}')

            #  Click button <Connect Wallet>
            if not press_button(self.driver,'Connect Wallet'):
                logger.error(f' (onchaingm), Failed to click button <Connect Wallet>')

            #  Click button <MetaMask>
            if not press_button(self.driver, 'MetaMask'):
                logger.error(f' (onchaingm), Failed to click button <MetaMask>')
            ...
            logger.error(f"Error in onchaingm activity: {str(e)}")
            return {
                'activity_type': 'Onchaingm',
                'status': 'error',
