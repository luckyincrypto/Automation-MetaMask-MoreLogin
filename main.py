...
class BrowserManager:
    ...
    async def start_browser_profile(env_id: str):
        ...
        try:
            ...
            logger.critical(f" (start_browser_profile) Error: {e}. Перезапустите программу. Сервер вернул не полный ответ.")
    ...
    for row in range(start_account, end_account + 1):
        ...
        try:
            ...
            if password is None and seed:
                ...
                logger.warning(f"[NEW PASSWORD] Для профиля № {unique_id} создан новый пароль.")

            if seed is None:
                seed, mm_address, private_key = create_wallet()
                password = worksheet_mm.cell(row=row+1, column=2).value = create_password()  # create a new password,
            ...
            profiles.append([unique_id, password, seed, mm_address, private_key, worksheet_mm, workbook_mm, row])
...
def get_user_input():
    ...
    if start_account != end_account:
        ...
        if delay_execution == "y":
            while True:
                delay_from_to = input("Введите задержку выполнения (от, до) секунд, через запятую: ").split(',')
                try:
    ...
    return start_account, end_account, mix_profiles, delay_from_to, mode_close_profile_or_not
...
async def operationEnv(driver, seed, env_id, password, mm_address, worksheet_mm, workbook_mm, row, env_name, unique_id):
    """Основная операция."""
    try:
        ...
        modify_file_runtimelavamoat(env_id)

        wallet_mm_from_browser_extension = meta_mask(driver, seed, password, env_id, mm_address, row, workbook_mm, worksheet_mm, FILE_PATH)
...
async def main_flow(env_id, seed, password, env_name, unique_id, mm_address, worksheet_mm, workbook_mm, row):
    ...
    try:
        while True:
            ...
            if not await operationEnv(driver, seed, env_id, password, mm_address, worksheet_mm, workbook_mm, row, env_name, unique_id):
                ...
                counter += 1
                if counter > 10:
                    logger.critical(f"\n{"#" * 20} (main_flow), (operationEnv), {counter} Не удачных запусков! Profile №: {unique_id}, Env_Name: {env_name}, Env ID: {env_id},"
                                f"Env ID: {env_id} завершён. Переход к следующему профилю. {"#" * 20}")
                    traceback.print_exc()
                    break
            else:
                break
        ...
        logger.error(f" (main_flow) ERROR in Profile №: {unique_id}, Env_Name: {env_name}, Env ID: {env_id}, Ошибка: {e}")
        traceback.print_exc()
    finally:
...
async def main():
    ...
    if os.path.exists(FILE_PATH):
        ...
        for row in range(2, 102):  # от 2 до 101 (включительно)
            worksheet_mm.cell(row=row, column=1, value=row - 1)  # Заполнение первой колонки нумерация от 1 до 100
        workbook_mm.save(FILE_PATH)  # Сохранение нового файла
        logger.info(f" (main) DATABASE created new file: {FILE_PATH}.")
    ...
    for profile in profiles:
        count_profile += 1
        unique_id, password, seed, mm_address, private_key, worksheet_mm, workbook_mm, row = profile
    ...
    logger.warning(f" (main) FINISH. Total time duration spent all profiles: {total_duration}, total profiles: {count_profile}\n{"-" * 90}\n")
